...
class LassoRegression(object):
    ...
    def SquareLoss(self, y, y_pred):
        '''区别就在损失函数不同'''
        return (np.sum((y - y_pred) ** 2)  + self.lambda_) / (len(y) * 2)

    def fit(self, X, y):
        w = np.random.rand(len(X[0]) + 1)  # b, a, 构造y = a*x + b
        ...
        for itr in range(self.max_itr):
            ...
            print(temp)
            w += self.lr_rate * (temp + self.lambda_)/(len(y)**2)
            # print(w)
            self.w = w
...
if __name__ == "__main__":
    ...
    rgs = LassoRegression(max_itr=100, lr_rate=1e-3, eps=0.01, lamda_ = 1)
